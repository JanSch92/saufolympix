...
class JokerController extends AbstractController
{
    ...
    public function useDoubleJoker(int $playerId, int $gameId): Response
    {
        ...
        $this->addFlash('success', 'Doppelte Punkte Joker für "' . $player->getName() . '" wurde aktiviert');
        return $this->redirectToRoute('app_game_admin', ['id' => $game->getOlympix()->getId()]);
    }
    ...
    public function useSwapJoker(int $playerId, int $gameId, Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($this->jokerRepository->hasSwapJokerOnPlayer($targetPlayer->getId(), $gameId)) {
                $this->addFlash('error', 'Tausch mit "' . $targetPlayer->getName() . '" ist bereits blockiert');
                return $this->redirectToRoute('app_game_admin', ['id' => $game->getOlympix()->getId()]);
            }
            ...
            $this->addFlash('success', 'Punkte tauschen Joker wurde aktiviert: "' . $player->getName() . '" <-> "' . $targetPlayer->getName() . '"');
            return $this->redirectToRoute('app_game_admin', ['id' => $game->getOlympix()->getId()]);
        }

        // Show swap selection form
        $availablePlayers = [];
    ...
    public function cancel(int $id): Response
    {
        ...
        if (!$joker->canBeUsed()) {
            $this->addFlash('error', 'Joker wurde bereits verwendet');
            return $this->redirectToRoute('app_joker_manage', ['gameId' => $joker->getGame()->getId()]);
        }

        $player = $joker->getPlayer();
        $gameId = $joker->getGame()->getId();

        // Reset player joker status
    ...
    public function apiSwapBlocked(int $playerId, int $gameId): Response
    {
        ...
        return $this->json([
            ...
            'message' => $isBlocked ? 'Tausch mit diesem Spieler ist blockiert' : 'Tausch möglich'
        ]);
    }
}